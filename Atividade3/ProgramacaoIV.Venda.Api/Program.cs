...
app.MapPost("/transacoes", async ([FromBody] TransacaoCapaRequest request, VendaContext context) =>
{
    var cliente = await context.Clientes
        .FirstOrDefaultAsync(x => x.Id == request.IdCliente && x.IsAtivo);

    var vendedor = await context.Vendedores
        .FirstOrDefaultAsync(x => x.Id == request.IdVendedor && x.IsAtivo);

    if (cliente is null || vendedor is null)
        return Results.NotFound("Cliente ou Vendedor inválido ou inativo.");
...
app.MapPost("/transacoes/{idTransacao}/itens", async (string idTransacao, [FromBody] TransacaoItemRequest request, VendaContext context) =>
{
    try
    {
        ...
        if (produto.Estoque < request.Quantidade)
            throw new InvalidOperationException($"Quantidade solicitada ({request.Quantidade}) excede o estoque disponível ({produto.Estoque}) para o produto {produto.EAN} - {produto.Descricao}.");
...
app.MapGet("/vendedores/{id}", async (string id, VendaContext context) =>
    await context.Vendedores.FirstOrDefaultAsync(x => x.Id == Guid.Parse(id) && x.IsAtivo) is Vendedor vendedor
        ? Results.Ok(vendedor)
        : Results.NotFound());

app.MapPut("/vendedores/{id}", async (string id, Vendedor input, VendaContext context) =>
{
    var vendedor = await context.Vendedores.FirstOrDefaultAsync(x => x.Id == Guid.Parse(id) && x.IsAtivo);
